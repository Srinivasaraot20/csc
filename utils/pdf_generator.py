...
def generate_certificate(student):
    ...
    try:
        ...
        draw_text(c, student.student_name, 490, img_height - 410, "Arial-Bold", 25, 'center')
        ...
        if hasattr(student, 'roll_number') and student.roll_number:
            c.drawString(916, img_height - 498, student.roll_number)

        # Program field (Internship Name): X: 734px, Y: 548px (from top-left) - Centered
        draw_text(c, student.internship_name, 734, img_height - 548, "Arial", 25, 'center')
        ...
        # Certificate ID field: X: 240px, Y: 160px (from bottom-left grid)
        c.setFont("Arial", 20)
        c.setFillColor(colors.red)
        c.drawString(240, 160, student.certificate_id)

        # Issue Date field: X: 180px, Y: 130px (from bottom-left grid)
        c.drawString(180, 130, datetime.now().strftime('%d-%m-%Y'))

        # Generate QR code (X:50, Y:120 from bottom-left grid)
        try:
        ...
        c.drawInlineImage(qr_temp_path, 50, 120, width=60, height=60)
